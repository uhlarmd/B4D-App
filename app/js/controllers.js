 ...
 *
 */
function MainCtrl($http, $scope, $state) {
...
function registerCtrl($http, $scope, $state, SweetAlert) {
    ...
    $scope.RegAgreed = false;
    $scope.register = function() {
        $http({
            url: "http://www.thedig.co/data/api/registerartist",
            method: 'POST',
            ...
            if(response.data.value === 1) {
                ...
                sessionStorage.setItem('group', JSON.stringify(response.data.group));
                SweetAlert.swal({
                    title: "Success!",
            ...
            if(response.data.value === 0) {
                ...
                SweetAlert.swal({
                    title: "Uh Oh!",
                    text: 'Username and password not recognized',
                    type: "warning",
...
function register2Ctrl($http, $scope, $state, SweetAlert) {
...
function loginCtrl($http, $scope, $state) {
    ...
    $scope.login = function() {
        $http({
            url: "http://www.thedig.co/data/api/login",
            method: 'POST',
            dataType: 'application/json',
            data: {
...
function dashCtrl($http, $scope, $state) {
    ...
    $scope.checkToken = function() {
        ...
        $http({
            url: "http://www.thedig.co/data/api/checktoken",
            method: 'POST',
            dataType: 'application/json',
            data: {
                token: token
...
function uploadCtrl($http, $scope, $state, SweetAlert, FileUploader) {
    ...
    function ekUpload(){
        ...
        function parseFile(file) {
            ...
            if (isGood) {
                ...
                document.getElementById('review-bad').classList.add("hidden");
                document.getElementById('review-block').classList.remove("hidden");
        ...
        function uploadFile(file) {
            ...
            if (xhr.upload) {
                ...
                if (file.size <= fileSizeLimit * 1024 * 1024) {
                    ...
                    $scope.form_data.append('file', file);
                    xhr.send(file);
                } else {
    ...
    $scope.uploadForm = function(){
        ...
        $scope.form_data.append('title', $scope.uploadData.title);
        $scope.form_data.append('description', $scope.uploadData.description);
        $scope.form_data.append('price', $scope.uploadData.price);
        $scope.form_data.append('areas', $scope.uploadData.areas);

        angular.forEach($scope.files, function(file){
            $scope.form_data.append('file', file);
        });
        $http.post('http://www.thedig.co/data/api/testimage', $scope.form_data,
            {
...
function orderCtrl($scope, $state, $http) {

    $http.get('http://www.thedig.co/data/api/totalartists').success(function(resp) {
        console.log(resp)
        $scope.total = 100 - resp.total;
    })

};

function overviewCtrl($scope, $state, $http, $rootScope, $fancyModal) {
    $scope.lines = $rootScope.lines.sort(
        function (a, b) { return b.poleDetection.Priority - a.poleDetection.Priority }
    );
    ...
    $scope.openModal = function (line) {
        $scope.line = line;
        $fancyModal.open({
            templateUrl: '../views/lineModal.html',
            controller: function ($scope) {
                $scope.line = line;
                console.log('modal line');
    ...
    $scope.ColorBars = function(inVal) {
        if (inVal == 3) {
            return "background-color: #FF3232; color: black";
        } else if (inVal == 2) {
            return "background-color: #FFFF26; color: black";
        } else if (inVal == 1) {
            return "background-color: #26FF26; color: black";
        } else {
            return "color: black";
        }
    };
...
function galleryCtrl($scope, Lightbox){
    $scope.images = [
        {
            'url': 'data/pole_0/1.png',
            'thumbUrl': 'data/pole_0/1.png' // used only for this example
        },
        {
            'url': 'data/pole_0/2.png',
            'thumbUrl': 'data/pole_0/2.png' // used only for this example
        },
        {
            'url': 'data/pole_0/3.png',
            'thumbUrl': 'data/pole_0/3.png' // used only for this example
        }
        // {
        //     'url': '2.gif',
        //     'thumbUrl': 'thumb2.jpg'
